...
public class WormholesSolution {
    public static void main(String[] args) {
        runTests();
        Scanner in = new Scanner(System.in);
        int numOfContests = in.nextInt();
        int numOfV = in.nextInt();
        int numOfW = in.nextInt();
        ArrayList<Contest> contests = new ArrayList<>();
        for (int i = 0; i < numOfContests; i++) {
            contests.add(new Contest(in.nextInt(), in.nextInt()));
        }
        TreeSet<Integer> startTimesOfV = new TreeSet<>();
        for (int i = 0; i < numOfV; i++) {
            startTimesOfV.add(in.nextInt());
        }
        TreeSet<Integer> startTimesOfW = new TreeSet<>();
        for (int i = 0; i < numOfW; i++) {
            startTimesOfW.add(in.nextInt());
        }
        System.out.println(getShortestDuration(contests,startTimesOfV,startTimesOfW));
    }

    public static int getShortestDuration(List<Contest> contests, TreeSet<Integer> startTimesOfV, TreeSet<Integer> startTimesOfW){

        int shortestTime = Integer.MAX_VALUE;
        for (Contest contest : contests) {
            Integer wormholeStartTime = startTimesOfV.floor(contest.start) ;
            Integer wormholeReturnTime = startTimesOfW.ceiling(contest.end) ;
            if(wormholeStartTime != null && wormholeReturnTime != null)
            shortestTime = Math.min(shortestTime,(wormholeReturnTime - wormholeStartTime) + 1);
        }
        return shortestTime;
    }
